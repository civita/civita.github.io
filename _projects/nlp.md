...
Note that here sentences are never inserted at the beginning of the paragraph due to the fact that the first sentence is expected somehow highly related to the topic. An example of the generated paragraph is given in Figure 2. Note that because distracting sentences no longer always locate at the end of contexts, the trained model based on this modified dataset will not just discard the last sentence.
